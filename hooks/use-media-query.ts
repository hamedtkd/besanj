...
function resolveMin(value: Breakpoint | number) {
  const px = typeof value === "number" ? value : BREAKPOINTS[value];
  return `(min-width: ${px}px)`;
}

function resolveMax(value: Breakpoint | number) {
  const px = typeof value === "number" ? value : BREAKPOINTS[value];
...
export function parseQuery(
  ...
  return parts.length ? parts.join(" and ") : query;
}
...
/** Copied from PersianLabs/ui. SSR-safe and accepts PersianLabs breakpoint syntax. */
